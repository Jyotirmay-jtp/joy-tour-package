...
export function PackageCard({ title, badge, desc, href, image, i }: { title: string; badge: string; desc: string; href: string; image: string; i: number }) {
  return (
    <motion.div {...fadeUp} transition={{ ...fadeUp.transition, delay: i * 0.05 }}
      className="group relative h-80 overflow-hidden rounded-3xl border border-white/10 shadow-[0_20px_50px_-20px_rgba(27,27,27,0.25)] ring-1 ring-black/5 transition-all duration-300 hover:ring-[#B91C1C]/30">
...
<div className="absolute inset-0 flex flex-col justify-end bg-black/0 p-6 pb-3 transition-all duration-300 group-hover:bg-black/60 group-hover:pb-6">        <h3 className="font-serif text-2xl text-white">{title}</h3>
