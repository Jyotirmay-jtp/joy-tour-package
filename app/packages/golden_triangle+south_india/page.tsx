"use client";
import { Clock, MapPin, Compass, Car, Users, Sparkles, ShieldCheck, Headphones, Camera, Utensils, Sailboat, ShoppingBag, Ship, Gem } from "lucide-react";
...
function Hero() {
  const infoCards = [
    { icon: Clock, label: "Duration", value: "18 Nights / 19 Days" },
    { icon: MapPin, label: "Destinations", value: "Delhi · Rajasthan · Aurangabad · Mumbai · Kerala" },
  ...
  return (
    <section className="relative flex min-h-[92vh] items-end overflow-hidden rounded-b-[48px] px-6 pb-16 pt-32 sm:px-10">
      <LazyVideo mobileSrc="/videos/south_mobile.mp4" desktopSrc="/videos/south.mp4" poster="/images/hero-poster.webp" />
      ...
      <div className="relative z-30 mx-auto w-full max-w-6xl">
        <motion.p {...fadeUp} className="text-sm font-medium text-white/70">Home / Packages / Golden Triangle + Rajasthan + South India</motion.p>
        ...
        <motion.h1 {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.1 }} className="mt-6 font-serif text-5xl leading-[1.05] text-white sm:text-7xl">
          Golden Triangle, Rajasthan & South India
        </motion.h1>
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.15 }} className="mt-2 font-serif text-2xl text-[#F5E7D3] sm:text-3xl">
          Delhi · Agra · Ranthambore · Jaipur · Narlai · Udaipur · Aurangabad · Mumbai · Kochi · Alleppey
        </motion.p>
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.2 }} className="mt-6 max-w-xl text-white/75">
          Mughal monuments, royal Rajasthan, ancient rock-cut caves, buzzing Mumbai and the Kerala backwaters — one grand circuit across the best of India.
        </motion.p>
...
function QuickOverview() {
  const cities = ["Delhi", "Agra", "Ranthambore", "Jaipur", "Narlai", "Udaipur", "Aurangabad", "Mumbai", "Kochi", "Alleppey"];
  const highlights = ["Taj Mahal", "Ranthambore Safari", "Ranakpur Jain Temple", "Ajanta & Ellora Caves", "Kerala Backwaters"];
  return (
    <section className="px-6 py-10 sm:px-10">
      <motion.div {...fadeUp} className="mx-auto grid max-w-6xl gap-4 rounded-[32px] bg-white p-6 shadow-[0_20px_60px_-25px_rgba(27,27,27,0.15)] sm:grid-cols-4 sm:p-8">
        <div>
          ...
          <p className="mt-2 font-serif text-2xl text-[#1B1B1B]">18 Nights / 19 Days</p>
        </div>
        <div>
...
export default function GoldenTriangleRajasthanSouthIndiaPage() {
  const included = [
  ...
  const optional = [
    { icon: Sailboat, label: "Houseboat Upgrade" }, { icon: Gem, label: "Elephant Ride" }, { icon: Utensils, label: "Local Food Tour" },
    { icon: Car, label: "Luxury Car Upgrade" }, { icon: ShoppingBag, label: "Shopping Tour" }, { icon: Ship, label: "Sunset Cruise" },
  ];
  const whyUs = [
  ...
  return (
    <main className="bg-[#FAFAF7] font-sans text-[#1B1B1B]">
      ...
      <CTA heading="Ready to experience Golden Triangle, Rajasthan & South India?" sub="Let us craft a personalized journey from the Taj Mahal to the Kerala backwaters, built exclusively for you." />
      <Footer />
      <WhatsAppButton />
    </main>
  );
