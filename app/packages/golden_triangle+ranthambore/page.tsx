...
function Hero() {
  const infoCards = [
    ...
    { icon: MapPin, label: "Destinations", value: "Delhi · Agra · Ranthambore · Jaipur" },
  ...
  return (
    <section className="relative flex min-h-[92vh] items-end overflow-hidden rounded-b-[48px] px-6 pb-16 pt-32 sm:px-10">
      <LazyVideo mobileSrc="/videos/ranthambore_mobile.mp4" desktopSrc="/videos/ranthambore.mp4" poster="/images/hero-poster.webp" />
      ...
      <div className="relative z-30 mx-auto w-full max-w-6xl">
        <motion.p {...fadeUp} className="text-sm font-medium text-white/70">Home / Packages / Golden Triangle + Ranthambore</motion.p>
        ...
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.2 }} className="mt-6 max-w-xl text-white/75">
          Mughal monuments, a royal tiger reserve, and the Pink City — a complete circuit through North India's icons and wilderness.
        </motion.p>
...
function QuickOverview() {
  const cities = ["Delhi", "Agra", "Ranthambore", "Jaipur"];
  const highlights = ["Taj Mahal", "Ranthambore Safari", "Amber Palace", "Chand Baori", "Fatehpur Sikri"];
  return (
...
export default function GoldenTriangleRanthamborePage() {
  const included = [
  ...
  const optional = [
    { icon: Sailboat, label: "Camel Safari" }, { icon: Gem, label: "Elephant Ride" }, { icon: Utensils, label: "Local Food Tour" },
    { icon: Car, label: "Luxury Car Upgrade" }, { icon: ShoppingBag, label: "Shopping Tour" }, { icon: Ship, label: "Boat Cruise" },
  ];
  const whyUs = [
  ...
  return (
    <main className="bg-[#FAFAF7] font-sans text-[#1B1B1B]">
      ...
      <CTA heading="Ready to experience Golden Triangle + Ranthambore?" sub="Let us create a personalized journey through Delhi, Agra, Ranthambore and Jaipur, crafted exclusively for you." />
      <Footer />
      <WhatsAppButton />
    </main>
  );
