"use client";
import { Clock, MapPin, Compass, Car, Users, Sparkles, ShieldCheck, Headphones, Camera, Mountain, Tent, ShoppingBag, Waves, FileCheck } from "lucide-react";
...
function Hero() {
  const infoCards = [
    { icon: Clock, label: "Duration", value: "7 Nights / 8 Days" },
    { icon: MapPin, label: "Destinations", value: "Leh · Nubra Valley · Pangong · Kargil · Srinagar" },
  ...
  return (
    <section className="relative flex min-h-[92vh] items-end overflow-hidden rounded-b-[48px] px-6 pb-16 pt-32 sm:px-10">
      <LazyVideo mobileSrc="/videos/leh_mobile.mp4" desktopSrc="/videos/leh.mp4" poster="/images/hero-poster.webp" />
      ...
      <div className="relative z-30 mx-auto w-full max-w-6xl">
        <motion.p {...fadeUp} className="text-sm font-medium text-white/70">Home / Packages / Leh and Srinagar</motion.p>
        ...
        <motion.h1 {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.1 }} className="mt-6 font-serif text-5xl leading-[1.05] text-white sm:text-7xl">
          Leh and Srinagar Tour
        </motion.h1>
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.15 }} className="mt-2 font-serif text-2xl text-[#F5E7D3] sm:text-3xl">
          Leh · Nubra Valley · Pangong Lake · Kargil · Srinagar
        </motion.p>
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.2 }} className="mt-6 max-w-xl text-white/75">
          High mountain passes, turquoise Himalayan lakes, ancient monasteries and the Mughal gardens of Kashmir — a complete circuit through Ladakh and the Kashmir Valley.
        </motion.p>
...
function QuickOverview() {
  const cities = ["Leh", "Nubra Valley", "Pangong Lake", "Kargil", "Srinagar"];
  const highlights = ["Khardung La Pass", "Pangong Tso", "Nubra Sand Dunes", "Magnetic Hill", "Dal Lake"];
  return (
    <section className="px-6 py-10 sm:px-10">
      <motion.div {...fadeUp} className="mx-auto grid max-w-6xl gap-4 rounded-[32px] bg-white p-6 shadow-[0_20px_60px_-25px_rgba(27,27,27,0.15)] sm:grid-cols-4 sm:p-8">
        <div>
          ...
          <p className="mt-2 font-serif text-2xl text-[#1B1B1B]">7 Nights / 8 Days</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className="mt-2 font-serif text-2xl text-[#1B1B1B]">May – September</p>
        </div>
      </motion.div>
    </section>
  );
...
export default function LehAndSrinagarPage() {
  const included = [
  ...
  const optional = [
    { icon: FileCheck, label: "Inner Line Permit Assistance" }, { icon: Tent, label: "Houseboat Stay in Srinagar" }, { icon: Mountain, label: "Camel Ride at Hunder Dunes" },
    { icon: Car, label: "Oxygen Cylinder On Request" }, { icon: ShoppingBag, label: "Kashmir Handicraft Shopping" }, { icon: Waves, label: "Shikara Ride on Dal Lake" },
  ];
  const whyUs = [
    { icon: ShieldCheck, label: "25+ Years Experience" }, { icon: Compass, label: "Local Ladakhi & Kashmiri Experts" }, { icon: Sparkles, label: "Flexible Tours" },
    { icon: MapPin, label: "Personalized Itineraries" }, { icon: Car, label: "Experienced High-Altitude Drivers" }, { icon: Headphones, label: "24×7 Support" },
  ];
  return (
    <main className="bg-[#FAFAF7] font-sans text-[#1B1B1B]">
      ...
      <CTA heading="Ready to experience Leh and Srinagar?" sub="Let us create a personalized journey through Leh, Nubra Valley, Pangong Lake, Kargil and Srinagar, crafted exclusively for you." />
      <Footer />
      <WhatsAppButton />
    </main>
  );
