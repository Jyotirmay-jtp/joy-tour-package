...
function Hero() {
  const infoCards = [
    ...
    { icon: MapPin, label: "Destinations", value: "Delhi · Agra · Aurangabad · Hampi" },
  ...
  return (
    <section className="relative flex min-h-[92vh] items-end overflow-hidden rounded-b-[48px] px-6 pb-16 pt-32 sm:px-10">
      <LazyVideo mobileSrc="/videos/hubli_mobile.mp4" desktopSrc="/videos/hubli.mp4" poster="/images/hero-poster.webp" />
      ...
      <div className="relative z-30 mx-auto w-full max-w-6xl">
        <motion.p {...fadeUp} className="text-sm font-medium text-white/70">Home / Packages / Delhi Agra Aurangabad Hampi</motion.p>
        ...
        <motion.h1 {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.1 }} className="mt-6 font-serif text-5xl leading-[1.05] text-white sm:text-7xl">
          Delhi, Agra, Aurangabad & Hampi
        </motion.h1>
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.15 }} className="mt-2 font-serif text-2xl text-[#F5E7D3] sm:text-3xl">
          Mughal Monuments · Rock-Cut Caves · Lost Empire of Vijayanagar
        </motion.p>
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.2 }} className="mt-6 max-w-xl text-white/75">
          The Taj Mahal, the UNESCO-listed caves of Ajanta and Ellora, and the boulder-strewn ruins of Hampi — a journey through India's ancient art and empires.
        </motion.p>
...
function QuickOverview() {
  const cities = ["Delhi", "Agra", "Aurangabad", "Mumbai", "Hampi"];
  const highlights = ["Taj Mahal", "Ajanta Caves", "Ellora Caves", "Hampi Ruins", "Vittala Temple"];
  return (
...
export default function DelhiAgraAurangabadHampiPage() {
  const included = [
  ...
  const optional = [
    { icon: Binoculars, label: "Hampi Coracle Ride" }, { icon: Gem, label: "Heritage Walk" }, { icon: Utensils, label: "Local Food Tour" },
    { icon: Car, label: "Luxury Car Upgrade" }, { icon: ShoppingBag, label: "Shopping Tour" }, { icon: Camera, label: "Sunset Photography" },
  ];
  const whyUs = [
  ...
  return (
    <main className="bg-[#FAFAF7] font-sans text-[#1B1B1B]">
      ...
      <CTA heading="Ready to experience Delhi, Agra, Aurangabad & Hampi?" sub="Let us craft a personalized journey from the Taj Mahal to the ruins of Hampi, built exclusively for you." />
      <Footer />
      <WhatsAppButton />
    </main>
  );
