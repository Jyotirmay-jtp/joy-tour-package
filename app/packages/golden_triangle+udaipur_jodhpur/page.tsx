"use client";
import { Clock, MapPin, Compass, Car, Users, Sparkles, ShieldCheck, Headphones, Camera, Utensils, ShoppingBag, Ship, Gem } from "lucide-react";
...
function Hero() {
  const infoCards = [
    { icon: Clock, label: "Duration", value: "9 Nights / 10 Days" },
    { icon: MapPin, label: "Destinations", value: "Delhi · Agra · Jaipur · Jodhpur · Udaipur" },
  ...
  return (
    <section className="relative flex min-h-[92vh] items-end overflow-hidden rounded-b-[48px] px-6 pb-16 pt-32 sm:px-10">
      <LazyVideo mobileSrc="/videos/jodhpur_mobile.mp4" desktopSrc="/videos/jodhpur.mp4" poster="/images/mehrangarh.webp" />
      ...
      <div className="relative z-30 mx-auto w-full max-w-6xl">
        <motion.p {...fadeUp} className="text-sm font-medium text-white/70">Home / Packages / Golden Triangle with Jodhpur & Udaipur</motion.p>
        ...
        <motion.p {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.2 }} className="mt-6 max-w-xl text-white/75">
          Mughal monuments, the Pink City, the Sun City's hilltop fort and the lakes of Udaipur — royal Rajasthan in full circuit.
        </motion.p>
...
function QuickOverview() {
  const cities = ["Delhi", "Agra", "Jaipur", "Jodhpur", "Udaipur", "Pushkar"];
  const highlights = ["Taj Mahal", "Amber Palace", "Mehrangarh Fort", "Lake Pichola", "Ranakpur Temple"];
  return (
    <section className="px-6 py-10 sm:px-10">
      <motion.div {...fadeUp} className="mx-auto grid max-w-6xl gap-4 rounded-[32px] bg-white p-6 shadow-[0_20px_60px_-25px_rgba(27,27,27,0.15)] sm:grid-cols-4 sm:p-8">
        <div>
          ...
          <p className="mt-2 font-serif text-2xl text-[#1B1B1B]">9 Nights / 10 Days</p>
        </div>
        <div>
...
export default function GoldenTriangleJodhpurUdaipurPage() {
  const included = [
  ...
  const optional = [
    { icon: Gem, label: "Elephant Ride" }, { icon: Ship, label: "Sunset Cruise" }, { icon: Utensils, label: "Local Food Tour" },
    { icon: Car, label: "Luxury Car Upgrade" }, { icon: ShoppingBag, label: "Shopping Tour" }, { icon: Camera, label: "Crystal Gallery Visit" },
  ];
  const whyUs = [
  ...
  return (
    <main className="bg-[#FAFAF7] font-sans text-[#1B1B1B]">
      ...
      <CTA heading="Ready to experience Golden Triangle with Jodhpur & Udaipur?" sub="Let us craft a personalized journey through royal Rajasthan, built exclusively for you." />
      <Footer />
      <WhatsAppButton />
    </main>
  );
